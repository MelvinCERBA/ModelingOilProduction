...
from functions import sigmoide, noised_sigmoide, grad_sigmoide, least_square,grad_least_square,scale_matrix
...
def test_grad_least_square(delta, Qmax=100, ts=10, tau=6, t_start=0, t_end=50):
    ...
    t = np.arange(t_start, t_end, 1)
    data = sigmoide(t,(Qmax+10,ts+10,tau+2))
...
def plot_isocurve_Qmax_fixed(percentage, Qmax=1, ts_init=50, tau_init=6, t_start=0, t_end=200):
    ...
    potentials = M[n//2,n//2] + [M[k*(n//2)//N_isocurve+n//2-1,n//2] for k in range(N_isocurve)]
    
    np.sort(potentials)

    Isocurve_tau = [[] for _ in range(N_isocurve)]

    Isocurve_ts = [[] for _ in range(N_isocurve)]

    # Seeking for isocurve
    for i in range(n):
        for j in range(n):
            for k,potential in enumerate(potentials[1:]):
                if abs(M[i,j]-potential)<(potentials[k+1]-potentials[k])**.5*N_isocurve*5/n:
                    Isocurve_tau[k].append(L_tau[i])  # y coordinate of the point of the isocurve
                    Isocurve_ts[k].append(L_ts[j])   # x coordinate

    plt.figure()
    plt.pcolormesh(L_ts,L_tau,M,shading='auto')
    
    # plot curves
    for k in range(N_isocurve):
        plt.scatter(Isocurve_ts[k],Isocurve_tau[k],marker=".", color="black")
    
    # plot direction
    k = 1
    N_point = 50
    for i in range(N_point):
        t = i*len(Isocurve_ts[k])//N_point
        ts_p, tau_p = Isocurve_ts[k][t],Isocurve_tau[k][t]
        
        d1 = -grad_least_square(sig, t_start, 1, sigmoide, grad_sigmoide, (Qmax,ts_p,tau_p))
        
        d2 = scale_matrix(t_start, t_end, grad_sigmoide, (Qmax,ts_p, tau_p)).dot(d1)
        
        plt.arrow(ts_p, tau_p,d2[1],d2[2], color = 'green', head_width=.3, width=0.1)
        
        plt.arrow(ts_p, tau_p,d1[1],d1[2], color = 'red', head_width=.3, width=0.1)
...
def plot_isocurve_tau_fixed(percentage, Smax_init=1, ts_init=50, tau=6, t_start=0, t_end=200):
    ...
    # window of value
    n = 500
    L_Qmax = np.linspace((1-percentage)*Smax_init,(1+percentage)*Smax_init,n)
    L_ts = np.linspace((1-percentage)*ts_init,(1+percentage)*ts_init,n)

    # Matrix of potential
    M = np.zeros((n,n))

    for i,Smax in enumerate(L_Qmax):
        ...
        for j in range(n):
            for k,potential in enumerate(potentials[1:]):
                if abs(M[i,j]-potential)<(potentials[k+1]-potentials[k])*2*N_isocurve/n:
                    ...
                    Isocurve_j[k].append(L_Qmax[j])


    plt.figure()
    plt.pcolormesh(L_Qmax,L_ts,M, shading='auto')
    for k in range(N_isocurve):
        plt.scatter(Isocurve_j[k],Isocurve_i[k],marker="_", label=f'isocourbe {k+1}')
    
    # Plots the center "+" 
    plt.plot(L_Qmax[n//2],L_ts[n//2],"+")
